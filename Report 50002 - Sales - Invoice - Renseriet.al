report 50002 "Sales - Invoice - Renseriet"
{
    // version NAVW113.00,NAVDK13.00,RENS1844.001

    DefaultLayout = RDLC;
    RDLCLayout = './Sales - Invoice - Renseriet.rdlc';
    Caption = 'Sales - Invoice Renseriet';
    ...
    dataset
    {
        dataitem("Sales Invoice Header"; "Sales Invoice Header")
        {
            ...
            column(BaseCalDesc; BaseCalDesc)
            {
            }
            column(JobNo; JobNo)
            {
            }
            column(InternalServiceTag; InternalServiceTag)
            {
            }
            column(SizeCategory; SizeCategory)
            {
            }
            ...
            begin
                ...
                JobNo := '';
                InternalServiceTag := '';
                IF BaseCalendar.GET(Cust."Base Calendar Code") THEN
                    BaseCalDesc := BaseCalendar.Name;

                JobLedgerEntry.RESET;
                JobLedgerEntry.SETRANGE("Document No.", "No.");
                IF JobLedgerEntry.FINDFIRST THEN BEGIN
                    JobNo := JobLedgerEntry."Job No.";
                    ServiceShipmentHeader.RESET;
                    ServiceShipmentHeader.SETRANGE("Order No.", JobLedgerEntry."Service Order No.");
                    IF ServiceShipmentHeader.FINDFIRST THEN BEGIN
                        ...
                        IF ServiceShipmentItemLine.FINDFIRST THEN BEGIN
                            IF ServiceItem.GET(ServiceShipmentItemLine."Service Item No.") THEN BEGIN
                                InternalServiceTag := ServiceItem."Internal Service Tag INE";
                                SizeCategory := ServiceItem."Size Category INE";
                            END;
                        END;
                    END;
                END;
    ...
    var
        ...
        InternalServiceTag: Code[20];
        SizeCategory: Code[20];
